...
class _BatchRunContext:
    ...
    # Per-document indexing token trackers, keyed by doc_id: (llm, embedding).
    #
    # A document crosses three long-lived worker tasks (parse → analyze →
    # process), so the trackers cannot live in a single task's contextvar for
    # the document's whole lifetime the way they did when one asyncio task
    # owned one document. Instead the pair is created once per doc_id here and
    # each stage rebinds the contextvars to it on entry (see
    # ``_bind_doc_token_trackers``), so usage from all three stages lands in
    # the same record. Entries are dropped at the terminal write.
    doc_token_trackers: dict = field(default_factory=dict)
...
class _PipelineMixin:
    ...
    # ============================================================
    # Per-document token attribution
    # ============================================================

    @staticmethod
    def _bind_doc_token_trackers(
        ctx: "_BatchRunContext", doc_id: str
    ) -> tuple["TokenTracker", "TokenTracker"]:
        """Bind this task's token contextvars to ``doc_id``'s tracker pair.

        Creates the pair on first use. Must be called at the top of every
        per-document iteration in EACH worker stage (parse / analyze /
        process): the workers are long-lived tasks that handle many documents
        in sequence, so the contextvar has to be re-pointed per document
        rather than set once per task. Provider calls downstream read these
        contextvars (see ``EmbeddingFunc.__call__`` and
        ``use_llm_func_with_cache``) and record usage without any call site
        having to thread a tracker through.

        Returns the ``(llm, embedding)`` pair so the process stage can read
        the totals at its terminal write.
        """
        trackers = ctx.doc_token_trackers.get(doc_id)
        if trackers is None:
            trackers = (TokenTracker(), TokenTracker())
            ctx.doc_token_trackers[doc_id] = trackers
        current_llm_tracker.set(trackers[0])
        current_embedding_tracker.set(trackers[1])
        return trackers
    ...
    async def _parse_worker(
        ...
        while True:
            ...
            try:
                ...
                # Attribute any parse-stage provider usage (VLM-backed parsers)
                # to this document before the engine runs.
                self._bind_doc_token_trackers(ctx, doc_id_w)
    ...
    async def _analyze_worker(self, ctx: _BatchRunContext) -> None:
        ...
        while True:
            ...
            try:
                ...
                # Attribute multimodal VLM usage to this document.
                self._bind_doc_token_trackers(ctx, doc_id_w)
                file_path_w = getattr(status_doc_w, "file_path", "unknown_source")
    ...
    async def _process_worker(self, ctx: _BatchRunContext) -> None:
        ...
        while True:
            ...
            try:
                ...
                # The document is terminal here (PROCESSED or FAILED, written
                # inside process_single_document), so its token totals have
                # already been read into doc_status metadata and the entry is
                # dead weight for the rest of the batch.
                try:
                    ctx.doc_token_trackers.pop(item[0], None)
    ...
    async def process_single_document(
        ...
        *,
        ...
        # Per-doc indexing token trackers. Captures the LLM and embedding usage
        # spent indexing this one document so the cost can be attributed
        # downstream. The pair is shared with the parse/analyze stages via
        # ctx.doc_token_trackers, so the totals below cover the whole document
        # lifecycle and not just this stage.
        doc_llm_tracker, doc_embedding_tracker = self._bind_doc_token_trackers(
            ctx, doc_id
        )
            ...
            try:
                ...
                if doc_process_opts.chunking_explicit:
                    ...
                    if strategy == "P":
                        ...
                        # into doc_status, where it can exceed a backend's value
                        # limit and leave the document permanently FAILED — even
                        # though the cascade the chunker actually used was empty.
                ...
                if blocks_path and sidecar_backfill_eligible:
                    ...
                    # Same reason as the chunkers themselves: this parses
                    # blocks.jsonl and then walks every block for every chunk, so
                    # it is O(chunks x blocks) of pure CPU plus synchronous file
                    # I/O — seconds of frozen event loop on a document with many
                    # chunks. It sits between chunking and the storage writes, so
                    # leaving it inline would keep /health stalled over exactly
                    # the window this work exists to free. One coarse submission,
                    # not one per chunk: the executor has a single worker and the
                    # backfill mutates ``chunking_result`` in place.
                    await run_in_chunking_executor(
                ...
                await self._finalize_doc_failure(
                    ...
                    metadata_extra={
                        ...
                        **truncation_metadata_extra(),
                        # Tokens spent before the failure are still real spend,
                        # so a failed document reports what it cost.
