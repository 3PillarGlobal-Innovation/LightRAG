        ...
        try:
            if binding == "lollms":
                ...
                if provider_supports_asymmetric and asymmetric_opt_in:
                    ...
                    if document_prefix:
                        kwargs["document_prefix"] = document_prefix
                # Only forwarded to providers whose embed function accepts it
                # (openai / azure_openai / gemini). jina, bedrock and voyageai
                # do not take token_tracker — passing it would TypeError.
                if token_tracker is not None:
                    kwargs["token_tracker"] = token_tracker
                return await actual_func(**kwargs)
        except ImportError as e:
...
def create_app(args):
    ...
    # Role LLM funcs/settings are workspace-independent, so they are resolved
    # once here and closed over by every per-workspace instance below.
    role_llm_configs = {
        spec.name: {
    ...
    def _build_rag(workspace):
        ...
        # Registered per instance, not just on the default one: a workspace
        # constructed lazily by the registry needs the same role-LLM builder,
        # otherwise role overrides silently fall back to the base LLM there.
    ...
    # Role provider options are identical across workspaces, so log them once
    # for the default instance rather than on every lazy construction.
