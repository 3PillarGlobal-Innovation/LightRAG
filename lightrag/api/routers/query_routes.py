...
class TokenUsageResponse(BaseModel):
    prompt_tokens: int = Field(default=0, description="Number of input tokens sent to the LLM")
    completion_tokens: int = Field(default=0, description="Number of output tokens generated by the LLM")
    total_tokens: int = Field(default=0, description="Total tokens (prompt + completion)")
    call_count: int = Field(default=0, description="Number of LLM calls made for this query")
    model_name: Optional[str] = Field(default=None, description="LLM model used for generation")
...
class QueryResponse(BaseModel):
    ...
    token_usage: Optional[TokenUsageResponse] = Field(
        default=None,
        description="LLM token usage statistics for cost calculation",
    )
        ...
        **Usage Examples:**
        ...
        try:
            ...
            if request.include_references:
                return QueryResponse(response=response_content, references=references, token_usage=token_usage)
            else:
                return QueryResponse(response=response_content, references=None, token_usage=token_usage)
